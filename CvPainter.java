...
{  private int maxX, maxY, centerX, centerY, w, h;
   ...
   {  
      try
      {
         for (;;)
         {  
        	 //update the light vector in dynamic
            Obj3D.sunX=1*Math.sin(60)*Math.cos(sunTheta);
            Obj3D.sunY=1*Math.sin(60)*Math.sin(sunTheta);
            sunTheta+=0.02;
            repaint();
            //animation draw speed
            Thread.sleep(20);
         }
      }
   ...
   {  if (obj == null) return;
      ...
      {  Tria tri = tr[iTria];
         ...
         int cCodeR,cCodeG,cCodeB;
         cCodeR=cCode+10<240?cCode+10:255;
         cCodeG=cCode+10<240?cCode+10:255;
         cCodeB=cCode;
         //setColor of the model using RGB
         gImage.setColor(new Color(cCodeR, cCodeG, cCodeB));
